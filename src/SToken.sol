...
import "./ShortControllerInterface.sol";
...
/**
 * @title ShortNFTProtocol 
 * @notice code base from Compound protocol(CToken.sol)
 * @author github@hoiJuwon
 */
contract SToken is STokenInterface, Exponential {
    /**
     * @notice Initialize the money market
     * @param shortController_ The address of the ShortController
     ...
     */
    function initialize(ShortControllerInterface shortController_,
        ...
        // Set the shortController
        uint err = _setShortController(shortController_);
        require(err == uint(Error.NO_ERROR), "setting shortController failed");

        // Initialize block number and borrow index (block number mocks depend on shortController being set)
    ...
    function transferTokens(address spender, address src, address dst, uint tokens) internal returns (uint) {
        ...
        uint allowed = shortController.transferAllowed(address(this), src, dst, tokens);
        if (allowed != 0) {
        ...
        // unused function
        // shortController.transferVerify(address(this), src, dst, tokens);
     ...
     * @dev This is used by shortController to more efficiently perform liquidity checks.
     ...
     */
    function mintInternal(uint erc1155TokenId, uint erc1155Amount) internal nonReentrant returns (uint, uint) {
        ...
        return mintFresh(msg.sender, erc1155TokenId, erc1155Amount);
    }
     ...
     * @return (uint, uint) An error code (0=success, otherwise a failure, see ErrorReporter.sol), and the actual mint amount.
     */
    function mintFresh(address minter, uint erc1155TokenId, uint erc1155Amount) internal returns (uint, uint) {
        /* Fail if mint not allowed */
        uint allowed = shortController.mintAllowed(address(this), minter, erc1155TokenId, erc1155Amount);
        if (allowed != 0) {
        ...
        doTransferIn(minter, erc1155TokenId, erc1155Amount);

        (vars.mathErr, vars.mintTokens) = divScalarByExpTruncate(vars.actualMintAmount, Exp({mantissa: vars.exchangeRateMantissa})); require(vars.mathErr == MathError.NO_ERROR, "MINT_EXCHANGE_CALCULATION_FAILED");
    ...
    function redeemFresh(address payable redeemer, uint redeemTokensIn, uint redeemAmountIn) internal returns (uint) {
        ...
        uint allowed = shortController.redeemAllowed(address(this), redeemer, vars.redeemTokens);
        if (allowed != 0) {
        ...
        shortController.redeemVerify(address(this), redeemer, vars.redeemAmount, vars.redeemTokens);
    ...
    function borrowFresh(address payable borrower, uint borrowAmount) internal returns (uint) {
        ...
        uint allowed = shortController.borrowAllowed(address(this), borrower, borrowAmount);
        if (allowed != 0) {
        ...
        /* We call the defense hook */
        // unused function
        // shortController.borrowVerify(address(this), borrower, borrowAmount);
    ...
    function repayBorrowFresh(address payer, address borrower, uint repayAmount) internal returns (uint, uint) {
        ...
        uint allowed = shortController.repayBorrowAllowed(address(this), payer, borrower, repayAmount);
        if (allowed != 0) {
        ...
        /* We call the defense hook */
        // unused function
        // shortController.repayBorrowVerify(address(this), payer, borrower, vars.actualRepayAmount, vars.borrowerIndex);
    ...
    function liquidateBorrowFresh(address liquidator, address borrower, uint repayAmount, STokenInterface sTokenCollateral) internal returns (uint, uint) {
        ...
        uint allowed = shortController.liquidateBorrowAllowed(address(this), address(sTokenCollateral), liquidator, borrower, repayAmount);
        if (allowed != 0) {
        ...
        (uint amountSeizeError, uint seizeTokens) = shortController.liquidateCalculateSeizeTokens(address(this), address(sTokenCollateral), actualRepayAmount);
        ...
        /* We call the defense hook */
        // unused function
        // shortController.liquidateBorrowVerify(address(this), address(sTokenCollateral), liquidator, borrower, actualRepayAmount, seizeTokens);
    ...
    function seizeInternal(address seizerToken, address liquidator, address borrower, uint seizeTokens) internal returns (uint) {
        ...
        uint allowed = shortController.seizeAllowed(address(this), seizerToken, liquidator, borrower, seizeTokens);
        if (allowed != 0) {
        ...
        /* We call the defense hook */
        // unused function
        // shortController.seizeVerify(address(this), seizerToken, liquidator, borrower, seizeTokens);
    ...
    /**
      * @notice Sets a new shortController for the market
      * @dev Admin function to set a new shortController
      * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
      */
    function _setShortController(ShortControllerInterface newShortController) public returns (uint) {
        // Check caller is admin
        if (msg.sender != admin) {
            return fail(Error.UNAUTHORIZED, FailureInfo.SET_COMPTROLLER_OWNER_CHECK);
        }

        ShortControllerInterface oldShortController = shortController;
        // Ensure invoke shortController.isShortController() returns true
        require(newShortController.isShortController(), "marker method returned false");

        // Set market's shortController to newShortController
        shortController = newShortController;

        // Emit NewShortController(oldShortController, newShortController)
        emit NewShortController(oldShortController, newShortController);
